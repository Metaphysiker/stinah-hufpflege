...
const initCornerstone = async () => {
  ...
  try {
    ...
    // Fit image to viewport container so it's never cut off
    cornerstone.resize(csViewport.value);
    cornerstone.fitToWindow(csViewport.value);

    const defaultViewport = cornerstone.getViewport(csViewport.value);
    if (defaultViewport && defaultViewport.scale > 0) {
      baseScale.value = defaultViewport.scale;
    }
...
// Update Cornerstone Viewport (W/L, Zoom, Invert)
const updateCornerstoneViewport = () => {
...
watch([windowCenter, windowWidth, invert, zoomScale, panX, panY], () => {
  if (viewerMode.value === "cornerstone") {
    updateCornerstoneViewport();
  } else if (viewerMode.value === "mobile") {
    renderMobileCanvas();
...
// Presets (Default, Bone, Deep Bone, Soft Tissue, High Contrast)
...
// Pan & Drag handlers for Cornerstone / Mobile Canvas
...
const onMouseMove = (e: MouseEvent) => {
  ...
  // Right button or Shift key: Adjust Window/Level
...
<template>
  <div class="dicom-viewer-wrapper" ref="viewerContainer">
    ...
    <v-card class="mb-3 elevation-2 rounded-lg">
      <v-card-title class="d-flex flex-wrap align-center justify-space-between ga-2 bg-grey-lighten-4 py-2 px-4">
        <div class="d-flex align-center ga-2">
          <v-chip color="primary" size="small" variant="flat">
            <v-icon start icon="mdi-medical-bag"></v-icon>
            DICOM
          </v-chip>
          <span class="text-subtitle-1 font-weight-medium text-truncate" style="max-width: 250px;">
            {{ fileKey }}
          </span>
        </div>
      ...
      <!-- Viewport Toolbar (Visible in Cornerstone & Mobile mode) -->
      <v-card-text v-if="viewerMode !== 'tags'" class="py-2 px-3 border-t">
        <div class="d-flex flex-wrap align-center justify-space-between ga-3">
          <!-- W/L Sliders & Presets -->
          <div class="d-flex align-center flex-wrap ga-2 flex-grow-1" style="min-width: 280px;">
            <div class="d-flex align-center ga-1" style="width: 190px;">
              ...
              <v-slider
                ...
                density="compact"
              ></v-slider>
              <span class="text-caption" style="width: 35px;">{{ Math.round(windowCenter) }}</span>
            </div>
            ...
            <div class="d-flex align-center ga-1" style="width: 190px;">
              ...
              <v-slider
                ...
                density="compact"
              ></v-slider>
              <span class="text-caption" style="width: 35px;">{{ Math.round(windowWidth) }}</span>
            </div>

            <!-- Presets -->
            <v-menu location="bottom">
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props" size="small" variant="outlined" density="comfortable">
              ...
              <v-list density="compact">
                ...
                <v-list-item @click="applyPreset('soft')">Weichgewebe (Soft Tissue)</v-list-item>
          ...
          <div class="d-flex align-center ga-1">
            <v-btn
              ...
              size="small"
              variant="text"
              @click="zoomScale = Math.min(5.0, zoomScale + 0.2)"
              title="Zoom In"
            ></v-btn>
            <v-btn
              ...
              size="small"
              variant="text"
              @click="zoomScale = Math.max(0.2, zoomScale - 0.2)"
              title="Zoom Out"
            ></v-btn>
            <v-btn
    ...
    <v-card class="elevation-3 overflow-hidden rounded-lg position-relative bg-black" style="min-height: 420px;">
      <!-- Loading Overlay -->
      <div v-if="loading" class="d-flex flex-column align-center justify-center position-absolute fill-height w-100 bg-black" style="z-index: 10;">
        <v-progress-circular indeterminate color="primary" size="50"></v-progress-circular>
        <span class="mt-3 text-grey-lighten-1">Lade DICOM Bilddaten...</span>
      </div>

      <!-- Error State -->
      <div v-else-if="errorMessage" class="d-flex flex-column align-center justify-center fill-height pa-6 text-center">
      ...
      <div
        ...
        class="viewport-container fill-height d-flex align-center justify-center cursor-grab"
        ...
        @touchend="onTouchEnd"
        style="width: 100%; height: 500px; touch-action: none; position: relative;"
      >
        <div
          ref="csViewport"
      ...
      <div v-if="!loading && !errorMessage && viewerMode === 'tags'" class="pa-4 bg-grey-darken-4 text-white overflow-y-auto" style="max-height: 500px;">
        ...
        <v-table density="compact" class="bg-grey-darken-3 text-white rounded">
          ...
          <tbody>
            <tr><td>Patienten Name</td><td>{{ meta.patientName }}</td></tr>
            <tr><td>Patienten ID</td><td>{{ meta.patientId }}</td></tr>
            <tr><td>Modalität (Modality)</td><td><v-chip size="x-small" color="primary">{{ meta.modality }}</v-chip></td></tr>
            <tr><td>Untersuchungsdatum (Study Date)</td><td>{{ meta.studyDate }}</td></tr>
            <tr><td>Auflösung (Rows x Columns)</td><td>{{ meta.columns }} x {{ meta.rows }} px</td></tr>
            <tr><td>Bits Allokiert / Gespeichert</td><td>{{ meta.bitsAllocated }} / {{ meta.bitsStored }} bits</td></tr>
            <tr><td>Pixel Representation</td><td>{{ meta.pixelRepresentation === 1 ? 'Vorzeichenbehaftet (Signed)' : 'Vorzeichenlos (Unsigned)' }}</td></tr>
            <tr><td>Photometric Interpretation</td><td>{{ meta.photometricInterpretation }}</td></tr>
            <tr><td>Rescale Slope / Intercept</td><td>{{ meta.rescaleSlope }} / {{ meta.rescaleIntercept }}</td></tr>
            <tr><td>Window Center / Width (W/C, W/W)</td><td>{{ meta.windowCenter }} / {{ meta.windowWidth }}</td></tr>
            <tr><td>Transfer Syntax UID</td><td class="text-caption">{{ meta.transferSyntaxUid }}</td></tr>
          </tbody>
        </v-table>
        <div class="mt-4 text-right">
          <v-btn color="primary" variant="flat" :href="presignedUrl" target="_blank" download>
    ...
    <!-- Footer Overlay info -->
    <div v-if="!loading && !errorMessage && viewerMode !== 'tags'" class="d-flex justify-space-between align-center px-2 py-1 text-caption text-grey">
      <span>W/C: {{ Math.round(windowCenter) }} | W/W: {{ Math.round(windowWidth) }} | Zoom: {{ Math.round(zoomScale * 100) }}%</span>
