import { IEntityWithIdentity } from "@/interfaces/IEntityWithIdentity";
...
export class EntityFinder {
  ...
  findByIdOrLocalID<T extends IEntityWithIdentity>(
    ...
    if (id) {
      const resultById = entitiesToBeSearched.find(
        (entity) => entity.id === id
      );
      if (resultById) {
        return resultById;
    ...
    if (localID) {
      const resultByLocalID = entitiesToBeSearched.find(
        (entity) => entity.localID === localID
      );
      if (resultByLocalID) {
